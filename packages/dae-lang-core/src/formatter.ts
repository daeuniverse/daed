 ...
 *
 * This is the single source of truth — both dae-editor and dae-lsp import from here.
 */
...
/**
 * Type-prefix keywords whose colons should NOT be spaced.
 * e.g. `geosite:cn` stays as-is, but `fallback: proxy` gets a space.
 */
const TYPE_KEYWORDS = new Set(['geosite', 'geoip', 'full', 'contains', 'regexp', 'ext', 'keyword', 'regex'])
...
export function formatLineContent(content: string): string {
  ...
  formatted = formatted.replace(RE_COLON_DECL, (_match, key: string, value: string) => {
    if (TYPE_KEYWORDS.has(key.toLowerCase())) {
      return `${key}:${value}`
    }
